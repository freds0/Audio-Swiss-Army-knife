...
def convert_flac_to_wav(input_filepath, output_filepath, dry_run=False):
    if dry_run:
...
def process_directory(input_dir, output_dir, dry_run=False):
    ...
    files = glob(join(input_dir, '*.flac'))
    
    for input_filepath in tqdm(files, desc="Converting FLAC to WAV"):
        output_filepath = join(output_dir, splitext(basename(input_filepath))[0] + '.wav')
        convert_flac_to_wav(input_filepath, output_filepath, dry_run)

def main():
    parser = argparse.ArgumentParser(description="Convert FLAC to WAV.")
