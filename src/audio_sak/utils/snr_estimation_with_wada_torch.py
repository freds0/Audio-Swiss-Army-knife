...
def wada_snr(wav, device):
    '''
    Direct blind estimation of the SNR of a speech signal (Torch version).
    '''
    # init
    eps = 1e-10
...
def main():
    parser = argparse.ArgumentParser(description="Estimate SNR using WADA-SNR (Torch).")
    ...
    files = glob(join(input_dir, "**", "*.wav"), recursive=True)
