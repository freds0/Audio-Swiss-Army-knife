...
def force_align(txt, wav_fn, lang="eng"):
    if ExecuteTask is None or TextGrid is None:
        logger.error("aeneas or textgrid not installed. Please install them.")
        return []
    ...
    try:
        ...
        tier = next((tier for tier in textgrid.tiers if tier.name == "Token"), None)
        
        if tier is None:
            logger.warning("No Token tier found in TextGrid.")
            return []
        ...
        # Cleanup
        if txt_fn.exists(): txt_fn.unlink()
        if tg_fn.exists(): tg_fn.unlink()

def main():
    parser = argparse.ArgumentParser(description="Aeneas Force Align Example")
    parser.add_argument('--text', required=True, help="Text to align")
    ...
    result = force_align(args.text, args.audio, args.lang)
    for res in result:
        print(res)
