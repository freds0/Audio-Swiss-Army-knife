...
def execute_download(youtube_link, output_dir):
    ...
    try:
        ...
        logger.info(f"Downloaded video from {youtube_link}")
    except Exception as e:
        logger.error(f"Failed to download {youtube_link}: {e}")

def main():
    parser = argparse.ArgumentParser(description="Download video from YouTube using yt-dlp.")
    parser.add_argument('--base_dir', default='./')
    parser.add_argument('--input_file', default='links.txt', help='Text file with YouTube links')
    parser.add_argument('--output_dir', default='videos', help='Output folder')
